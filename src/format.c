...
void bl_format_address(uint64_t address, char *buffer, size_t buffer_size)
{
    ...
    snprintf(
        ...
        "0x%08llX",
        (unsigned long long)
            address); /* Use snprintf so small buffers are safely truncated instead of overflowing. */
}
...
void bl_format_u64_size(uint64_t bytes, char *buffer, size_t buffer_size)
{
    ...
    if (bytes >=
        1024ull * 1024ull *
            1024ull) { /* Choose the largest supported unit that keeps the displayed value readable. */
        snprintf(buffer,
                 buffer_size,
                 "%.1f GiB",
