...
typedef struct
    RegionBuilder { /* Temporary builder used to assemble one contiguous reconstructed memory region. */
...
static int compare_chunks(
    const void *left,
    const void *
        right) /* Sorts chunks by address, then end address and source line for deterministic reconstruction. */
{
...
static int ensure_region_capacity(
    BlFirmwareImage *image,
    BlDiagnostic *
        diag) /* Ensures the reconstructed region table has room for one more entry. */
{
    if (image->region_count == image->region_capacity) {
        size_t next_capacity =
            image->region_capacity == 0 ? 4 : image->region_capacity * 2;
        ...
        next_regions =
            (BlMemRegion *)realloc(image->regions,
                                   next_capacity * sizeof(*next_regions));
        if (next_regions == NULL) {
            bl_diag_set(diag,
                        BL_DIAG_ERROR,
                        "out of memory while growing region table");
            return -1;
        }
...
static int ensure_gap_capacity(
    BlFirmwareImage *image,
    BlDiagnostic *
        diag) /* Ensures the gap table has room for one more unloaded address range. */
{
    if (image->gap_count == image->gap_capacity) {
        size_t next_capacity =
            image->gap_capacity == 0 ? 4 : image->gap_capacity * 2;
        BlMemGap *next_gaps;
        ...
        next_gaps = (BlMemGap *)realloc(image->gaps,
                                        next_capacity * sizeof(*next_gaps));
        if (next_gaps == NULL) {
            bl_diag_set(diag,
                        BL_DIAG_ERROR,
                        "out of memory while growing gap table");
            return -1;
        }
...
static int ensure_overlap_capacity(
    BlFirmwareImage *image,
    BlDiagnostic *
        diag) /* Ensures the overlap table has room for one more overlap diagnostic. */
{
    if (image->overlap_count == image->overlap_capacity) {
        size_t next_capacity =
            image->overlap_capacity == 0 ? 4 : image->overlap_capacity * 2;
        ...
        next_overlaps =
            (BlMemOverlap *)realloc(image->overlaps,
                                    next_capacity * sizeof(*next_overlaps));
        if (next_overlaps == NULL) {
            bl_diag_set(diag,
                        BL_DIAG_ERROR,
                        "out of memory while growing overlap table");
            return -1;
        }
...
static int add_gap(
    BlFirmwareImage *
        image, /* Records an unloaded inclusive address range between two reconstructed regions. */
    uint64_t start_address,
    uint64_t end_address,
    BlDiagnostic *diag)
{
...
static int add_overlap(
    BlFirmwareImage *
        image, /* Records an overlap between two source chunks, preserving chunk indexes and source lines. */
    size_t first_index,
    size_t second_index,
    uint64_t start_address,
    uint64_t end_address,
    ...
    BlDiagnostic *diag)
{
    ...
    if (length >
        (uint64_t)
            SIZE_MAX) { /* Reject overlaps that cannot be represented safely as a platform size_t length. */
        bl_diag_set(diag,
                    BL_DIAG_ERROR,
                    "overlap length exceeds platform size limit");
        return -1;
    }
...
static int add_region_take(
    BlFirmwareImage *image,
    RegionBuilder *builder,
    BlDiagnostic *
        diag) /* Transfers the builder buffer into the image as a finalized reconstructed region.  */
{
    BlMemRegion *region;
    ...
    region->entropy = bl_entropy_shannon(
        builder->bytes,
        builder
            ->length); /* Store entropy once per region so report generation does not need to recompute it. */

    builder->bytes =
        NULL; /* Ownership of builder -> bytes has moved to the region entry. */
    builder->length = 0;
    builder->capacity = 0;
    return 0;
}

static int builder_init_from_chunk(
    RegionBuilder *
        builder, /* Starts a region builder from a source chunk by copying its bytes. */
    ...
    BlDiagnostic *diag)
{
    ...
    if (builder->bytes == NULL) {
        bl_diag_set(diag,
                    BL_DIAG_ERROR,
                    "out of memory while creating memory region");
        return -1;
    }
...
static int builder_append(
    RegionBuilder *
        builder, /* Appends bytes to the current region builder, growing its buffer if needed. */
    ...
    BlDiagnostic *diag)
{
    ...
    if (length > SIZE_MAX - builder->length) {
        bl_diag_set(diag,
                    BL_DIAG_ERROR,
                    "memory region length exceeds platform size limit");
        return -1;
    }

    if (builder->length + length > builder->capacity) {
        size_t next_capacity =
            builder->capacity == 0 ? length : builder->capacity;
        unsigned char *next_bytes;

        while (
            next_capacity <
            builder->length +
                length) { /* Grow geometrically, but fall back to the exact required size on overflow risk. */
        ...
        if (next_bytes == NULL) {
            bl_diag_set(diag,
                        BL_DIAG_ERROR,
                        "out of memory while extending memory region");
            return -1;
        }
...
static int append_non_overlapping_tail(
    RegionBuilder *
        builder, /* Appends only the non-overlapping suffix of a chunk already merged into a region. */
    const BlSourceChunk *chunk,
    BlDiagnostic *diag)
{
    uint64_t tail_start;
    ...
    tail_start =
        builder->end_address +
        1u; /* The tail begins immediately after the current region's inclusive end address. */
    ...
    if (offset > (uint64_t)SIZE_MAX || tail_length > (uint64_t)SIZE_MAX) {
        bl_diag_set(diag,
                    BL_DIAG_ERROR,
                    "memory region tail exceeds platform size limit");
        return -1;
    }
...
static int chunks_are_conflicting(
    const BlSourceChunk *
        first, /* Compares overlapping byte ranges to classify them as indetical or conflicting. */
    ...
    int *conflicting)
{
    ...
    if (first_offset > (uint64_t)SIZE_MAX ||
        second_offset > (uint64_t)SIZE_MAX || length > (uint64_t)SIZE_MAX) {
        bl_diag_set(diag,
                    BL_DIAG_ERROR,
                    "overlap comparison exceeds platform size limit");
        return -1;
    }
...
static int detect_overlaps(
    BlFirmwareImage *image,
    BlDiagnostic *
        diag) /* Finds all parawise chunk overlaps after chunks have been sorted by start address. */
{
    size_t i;
    ...
    for (i = 0; i < image->chunk_count; i++) {
        ...
        for (
            j = i +
                1u; /* Stop scanning once later chunks start beyond the chunk's end address. */
            j < image->chunk_count &&
            image->chunks[j].start_address <= image->chunks[i].end_address;
            j++) {
            uint64_t start =
                image->chunks[i].start_address > image->chunks[j].start_address
                    ? image->chunks[i].start_address
                    : image->chunks[j].start_address;
            uint64_t end =
                image->chunks[i].end_address < image->chunks[j].end_address
            ...
            if (add_overlap(image,
                            ...
                            conflicting ? BL_OVERLAP_CONFLICTING
                                        : BL_OVERLAP_IDENTICAL,
                            diag) != 0) {
                return -1;
            }
        }
...
static int can_merge(
    ...
    uint64_t
        next_start) /* Chunks can merge when they overlap or are directly adjacent. */
{
    return next_start <= current_end ||
           (current_end != UINT64_MAX && next_start == current_end + 1u);
}
...
int bl_memmap_reconstruct(BlFirmwareImage *image, BlDiagnostic *diag)
{
    ...
    bl_firmware_image_clear_layout(
        image); /* Drop any previous derived layout before rebuilding it from the current chunks. */
    ...
    for (i = 1; i < image->chunk_count; i++) {
        ...
        if (can_merge(
                builder.end_address,
                chunk
                    ->start_address)) { /* Merge overlapping or adjacent chunks into the current reconstructed region. */
        ...
        if (add_gap(
                image,
                image->regions[image->region_count - 1u].end_address +
                    1u, /* A gap exists between the finalized region and the next chunk start. */
                chunk->start_address - 1u,
                diag) != 0) {
            goto fail;
        }
